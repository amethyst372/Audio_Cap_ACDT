...
class AACDatasetBART(torch.utils.data.Dataset):
    def __init__(self, settings, data_dir, split, tokenizer):
        ...
        self.adjmat_name = settings['data']['adjmat_field_name']
    ...
    def __getitem__(self, item):
        ...
        fname = Path(ex).name
        ex = np.load(str(ex), allow_pickle=True)    # numpy rec.array
        
        # ----- Load adj_mat -----
        adj_mat = ex[self.adjmat_name].item()
