...
def main(config):
    ...
    # Training arguments
    out_dir = '/data/qust-011/saved/baseline/'+config.exp+'_out'
    ...
    if settings['workflow']['evaluate'] or settings['workflow']['infer']:
        ...
        if settings['lm']['eval_model'] == 'checkpoint': # Specific checkpoint
            model.load_state_dict(torch.load(out_dir+'/checkpoint-'+str(settings['lm']['eval_checkpoint'])+'/pytorch_model.bin', map_location=device), strict=False)
            ...
            model.load_state_dict(torch.load(out_dir+'/pytorch_model_best.bin', map_location=device), strict=False)
            ...
            model.load_state_dict(torch.load(settings['lm']['eval_model'], map_location=device), strict=False)
