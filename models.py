...
from dep_bart import DepBartGeneration
...
class BARTAAC(nn.Module):
    def __init__(self, settings, device):
        ...
        #     print('loading pretrained model from', pretrained_lm)
        ...
        if pretrained_lm is not None: # Bypass model configuration to load a pre-trained model (e.g. facebook/bart-base)
            print('loading pretrained model from', pretrained_lm)
            self.bart_lm = DepBartGeneration.from_pretrained(pretrained_lm)
        else:
            self.bart_lm = DepBartGeneration(bart_config)
        # MODIFED END
    ...
    def forward(self,
        ...
        # Decoder-only pass
        if self.settings['dep']['use']:
            outputs, inner_loss = self.bart_lm(input_ids=None,
                    attention_mask=attention_mask,
            ...
            return outputs['loss'], outputs['logits'], inner_loss
        else:
    ...
    def generate_beam(self,
        ...
        gen_config = GenerationConfig(
            max_length = self.settings['lm']['generation']['max_length'],
            min_length = self.settings['lm']['generation']['min_length'],
            early_stopping = self.settings['lm']['generation']['early_stopping'],
            num_beams = self.settings['lm']['generation']['num_beams'],
            no_repeat_ngram_size = self.settings['lm']['generation']['no_repeat_ngram_size'],
        )
        
        # MODIFIED
        if self.settings['dep']['use']:
            decoder_inputs_embeds = self.bart_lm.model.decoder.embed_tokens(input_ids) * \
                                self.bart_lm.model.decoder.embed_scale
            dep_prior, dep_post = self.bart_lm.model.decoder.get_dep(decoder_inputs_embeds.clone().detach(), None, audio_embs)   # use clone() to avoid being affected
            # decoder_inputs_embeds[:,0,:] = dep_prior

            decoder_attention_mask = self.bart_lm.model.decoder._prepare_decoder_attention_mask(
                attention_mask, input_ids.shape, decoder_inputs_embeds, 0
            )
