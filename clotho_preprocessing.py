...
sys.path.append('dep_parser')
from dep_parser import text2depAdj

def preprocess_dataset(config):
    with open('data_settings/default.yaml', 'r') as f:
        settings = yaml.safe_load(f)
    
    vggish_model = torch.hub.load('torchvggish-master/torchvggish', 'vggish', source='local')
    vggish_model.eval()
    ...
    for split in splits:
        ...
        if annotation_path.exists():
            ...
            for ex in tqdm(audio_list):
                ...
                # Create recarray
                temp_rec_array = np.rec.array(np.array(
                    (file_name, vggish_embeddings, None, None),
                    dtype=[
                        ('file_name', 'U{}'.format(len(file_name))),
                        ('vggish_embeddings', np.dtype(object)),
