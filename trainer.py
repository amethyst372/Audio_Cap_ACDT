...
class BARTAACTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        inner_loss = None
        if settings['dep']['use']:
            loss, outputs, inner_loss = model(**inputs)
        else:
        ...
        # if inner_loss:
        #     total_loss = (1-settings['dep']['inner_loss_frac']) * loss + \
        #         settings['dep']['inner_loss_frac'] * inner_loss
        # else:
        #     total_loss = loss
        total_loss = loss
        ...
        return (total_loss, outputs) if return_outputs else total_loss # Do not return past key values
    ...
    def caption_evaluate(self, eval_dataset, tokenizer, generation_mode='beam'):
        ...
        for metric, values in metrics.items():
            print(f'beam search: {metric:<7s}: {values["score"]:7.4f}')
